...
model_run_name = "Random_Forest_on_pruned_Tree_RS42"
...
for pisa_type in pbar_pisa:
    ...
    if pisa_type == "SCIENCE":
        input_data = "aggregated-years"
        iteration = 50
        alpha = 12.0344320544575
    else:
        input_data = "pisa-years"
        if pisa_type == "READ":
            iteration = 65
            alpha = 9.74069642649852
        else:
            iteration = 55
            alpha = 1.87235621521481

    model_run = model_run_name

    #get optimal params without pruning
    optimal_params = pd.read_csv(Path(path_results / f"{input_data}_Optimal_Single_Tree_RS42") /
                                 f"{pisa_type}_Overview_{input_data}_Optimal_Single_Tree_RS42.csv")
    ...
    X_train_final, X_test_final, y_train_final, y_test_final = train_test_split(X, y, test_size=0.15,
                                                                                random_state=42)
    # create model
    model_forest = RandomForestRegressor(random_state=42, n_jobs=-1, max_depth=max_depth,
                                         min_samples_leaf=min_leaf, min_samples_split=min_split, ccp_alpha=alpha)
