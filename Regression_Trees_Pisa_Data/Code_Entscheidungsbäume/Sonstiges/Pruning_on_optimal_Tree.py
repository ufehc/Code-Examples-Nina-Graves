...
model_run_name = "Pruning_on_Opt_Tree_RS42_2"
randomSearch = False
gridSearch = False
...
for pisa_type in pbar2:
    ...
    # load Dataframe with Data about optimal single Tree
    optimal_params = pd.read_csv(Path(path_results / f"{input_data}_Optimal_Single_Tree_RS42_2") /
                                 f"{pisa_type}_Overview_{input_data}_Optimal_Single_Tree_RS42_2.csv")
    ...
    # identify efficient Alphas
    model_tree = DecisionTreeRegressor(random_state=42, max_depth=max_depth, min_samples_leaf=min_leaf,
    ...
    params_opt = model_tree.get_params()
    ...
    for ccp_alpha in ccp_alphas:
        model_tree = DecisionTreeRegressor(max_depth=max_depth, min_samples_leaf=min_leaf,
                                           min_samples_split=min_split, ccp_alpha=ccp_alpha, random_state=42)
        ...
        results_with_total_overview.loc[model_run, "min_leafs"] = params_opt['min_samples_leaf']
        results_with_total_overview.loc[model_run, "min_splits"] = params_opt['min_samples_split']
        ...
        plt.legend(loc=1)
        plt.title(f"{pisa_type}-{input_data}_{iteration}: True Scores vs. Predictions - Alpha: {ccp_alpha}")
        ...
        plt.plot(range(len(data)), data["Annual Mean"], marker="x", c="r",
                 label="NAs approximated by annual mean")
    ...
    for ccp_alpha in ccp_alphas:
        ...
        results_comparative_modelrun_alphas.loc[ccp_alpha, "max_Depth"] = results_df.loc[model_run, "max_Depth"]
        results_comparative_modelrun_alphas.loc[ccp_alpha, "min_leafs"] = results_df.loc[model_run, "min_leafs"]
        results_comparative_modelrun_alphas.loc[ccp_alpha, "min_splits"] = results_df.loc[model_run, "min_splits"]
        ...
        for number in range(0, len(top_features)):
            results_comparative_modelrun_alphas.loc[ccp_alpha, f"Top {number + 1} feature"] = top_features[number]
...
for pisa_type in Pisa_Type:
    ...
    overview_iteration = overview_iteration.drop_duplicates(["ccp_alphas"])
    overview_iteration = overview_iteration.set_index("ccp_alphas")

    index = list(itertools.product([pisa_type], overview_iteration["Alpha"]))
    ...
    for alpha_it in overview_iteration.index:
        frame_iteration.loc[(pisa_type, alpha_it), "Percentage non-NA"] = iteration
        frame_iteration.loc[(pisa_type, alpha_it), "Alpha"] = alpha_it
        frame_iteration.loc[(pisa_type, alpha_it), "RMSE"] = overview_iteration.loc[alpha_it, "RMSE"]
        frame_iteration.loc[(pisa_type, alpha_it), "R2"] = overview_iteration.loc[alpha_it, "R2"]
        frame_iteration.loc[(pisa_type, alpha_it), "max_Depth"] = overview_iteration.loc[alpha_it, "max_Depth"]
        frame_iteration.loc[(pisa_type, alpha_it), "min_Leaf"] = overview_iteration.loc[alpha_it, "min_leafs"]
        frame_iteration.loc[(pisa_type, alpha_it), "min_split"] = overview_iteration.loc[alpha_it, "min_splits"]
        frame_iteration.loc[(pisa_type, alpha_it), "Nodes_Tree"] = overview_iteration.loc[
            alpha_it, "Nodes_Tree"]
        frame_iteration.loc[(pisa_type, alpha_it), "Impurities"] = overview_iteration.loc[alpha_it, "Impurity"]
        frame_iteration.loc[(pisa_type, alpha_it), "Features Tree"] = overview_iteration.loc[
            alpha_it, "Features Tree"]
        frame_iteration.loc[(pisa_type, alpha_it), "Total Features"] = overview_iteration.loc[
            alpha_it, "Total Features"]
        frame_iteration.loc[(pisa_type, alpha_it), "Percentage_NA_mean"] = overview_iteration.loc[
            alpha_it, "Percentage_NA_mean"]
        frame_iteration.loc[(pisa_type, alpha_it), "Mean_Approximated_Values[%]"] = overview_iteration.loc[
            alpha_it, "Mean_Approximated_Values[%]"]
        for number in range(0, 10):
            if f"Top {number + 1} feature" in overview_iteration.columns:
                frame_iteration.loc[(pisa_type, alpha_it), f"Top {number + 1} feature"] = overview_iteration.loc[
                    alpha_it, f"Top {number + 1} feature"]

        results_comparative_modelrun = results_comparative_modelrun.append(frame_iteration)

results_comparative_modelrun.to_csv(path_results_modelrun / f"Overview_{model_run}.csv")
