...
Pisa_Type = ["MATH"]
Model_Run = ["n_estimators", "max_depth", "max_features"]
Hyper_Parameter = {"n_estimators": [50, 100, 200, 250, 500, 1000],
...
for pisa_type in pbar_pisa:
    ...
    for model_run in pbar1:
        pbar1.set_description(f"{pisa_type}_{model_run}")

        parambar = tqdm(Hyper_Parameter[model_run])
        for h_param in parambar:
            parambar.set_description(f"{pisa_type}_{model_run}: {h_param}")

            # create directories
            path_results.mkdir(exist_ok=True)
            path_results_modelrun = Path(path_results / f"Random-Forest_Param_{model_run}")
            path_results_modelrun.mkdir(exist_ok=True)
            path_results_modelrun.mkdir(exist_ok=True)
            path_tree.mkdir(exist_ok=True)
            path_tree_modelrun = Path(path_tree/f"Random-Forest_Param_{model_run}")
            ...
            # create model
            if model_run == "n_estimators":
                model_forest = RandomForestRegressor(n_estimators=h_param, n_jobs=-1)
            if model_run == "max_depth":
                model_forest = RandomForestRegressor(max_depth=h_param, n_jobs=-1)
            if model_run == "max_features":
                model_forest = RandomForestRegressor(max_features=h_param, n_jobs=-1)
            ...
            features_merged.to_csv(path_results_modelrun / f"{pisa_type}_Feature_Analysis_{model_run}_{h_param}.csv")
            ...
            results["R^2"] = np.nan
            new_row_df = pd.DataFrame(columns=["Modelrun", "COUNTRY", "YEAR", "PISA-SCORE", "y_predict", "Absoloute Error", "Squared Error", "RMSE", "R^2"],
            ...
            results_with_total_overview.to_csv(path_results_modelrun / f"{pisa_type}_Results_{model_run}_{h_param}.csv")
            ...
            for estimator in child_estimators:
                ...
                child_estimator_info.loc[estimator, "Parameter-Setting"] = h_param
                ...
                child_estimator_info.loc[estimator, "Feature Importance"] = str(estimator.feature_importances_) # return to list with json lib -> json.loads()
                child_estimator_info.loc[estimator, "Predictions"] = str(estimator.predict(X_test_final)) # return to list with json lib -> json.loads()
            child_estimator_info.to_csv(path_results_modelrun / f"{pisa_type}_Tree-Childen-Info_{model_run}_{h_param}.csv")
            ...
            plt.legend(loc=1)
            plt.savefig(path_results_modelrun_plots / f"RESULT_{pisa_type}_{model_run}_{h_param}.png")
            plt.close()
            ...
            plt.ylabel("RMSE")
            plt.savefig(path_results_modelrun_plots / f"{pisa_type}_RMSE-True_Score_{model_run}_{h_param}.png")
            plt.close()
            ...
            plt.savefig(path_results_modelrun_plots / f"{pisa_type}_RMSE_per_year_{model_run}_{h_param}.png")
            plt.close()
            ...
            plt.plot(range(len(feature_names)), feature_names["Annual Mean"], marker="x", c="r", label="Annual Mean used for NaN")
            plt.hist(feature_names.index, bins=range(len(feature_names)), color="slategray", align="left", rwidth=0.5,
                     weights=feature_names["NA-Values"], label="# NA-Values")
            plt.legend()
            plt.xticks(range(len(feature_names.index)), range(len(feature_names)));
            plt.xlabel(feature_names, fontsize="x-small", ma="left")
            plt.gcf().subplots_adjust(bottom=0.5)
            plt.savefig(path_results_modelrun_plots / f"{pisa_type}_Robustness_Check_{model_run}_{h_param}.png")
            plt.close()

            ###Feature Analysis###
            Importance_Measures = ["Feature Importance", "TEST-Permutation Importance Mean", "TRAIN-Permutation Importance Mean"]
            ...
            for importance_measure in Importance_Measures:
                ...
                plt.savefig(path_results_modelrun_plots /
                            f"{pisa_type}_{importance_measure}_{model_run}_{h_param}.png")
                plt.close()
            ...
            plt.title(f"{pisa_type}-{input_data}_{iteration}: Measures of Feature Importance: {model_run}={h_param}")
            ...
            plt.scatter(x=range(len(names)), y=plot_df["TRAIN-Permutation Importance Mean"], label="TRAIN Permutation Importance")
            plt.xticks(ticks=range(len(names)), fontsize="small")
            plt.xlabel(names_overview, fontsize="x-small", ma="left")
            plt.legend(loc="upper left", fontsize="small")
            plt.gcf().subplots_adjust(bottom=0.3)
            fig.savefig(path_results_modelrun_plots / f"{pisa_type}_Overview_Importance_Measures_{model_run}_{h_param}.png")
            plt.close()
            ...
            for country in  estimated_countries:
                ...
                plt.savefig(path_results_modelrun_plots / f"{pisa_type}_Plot_{country}_{model_run}_{h_param}.png")
                plt.close()
            ...
            plt.ylabel("Score")
            plt.legend(loc=4)
            plt.savefig(path_results_modelrun_plots/ f"{pisa_type}_Plot_all_countries_{model_run}_{h_param}.png")
            plt.close()

index = []
for model_run in Model_Run:
    tmp = list(itertools.product(Hyper_Parameter[model_run], [model_run], Pisa_Type))
    index.extend(tmp)
results_comparative_modelrun = pd.DataFrame(index=pd.MultiIndex.from_tuples(index))
...
for model_run in Model_Run:
    ...
    for h_param in Hyper_Parameter[model_run]:
        ind = (h_param, model_run, pisa_type)

        features_df = pd.read_csv(
            path_results_modelrun / f"{pisa_type}_Feature_Analysis_{model_run}_{h_param}.csv")
        features_df = features_df.set_index("Unnamed: 0")
        results_df = pd.read_csv(path_results_modelrun / f"{pisa_type}_Results_{model_run}_{h_param}.csv")
        ...
        results_comparative_modelrun.loc[ind, "Pisa-Type"] = pisa_type
        results_comparative_modelrun.loc[ind, "Percentage non-NA"] = iteration
        results_comparative_modelrun.loc[ind, "Percentage non-NA"] = iteration
        results_comparative_modelrun.loc[ind, f"Model_Run"] = model_run
        ...
        results_comparative_modelrun.loc[ind, "Total Features"] = results_df.loc[
            model_run, "Total available features"]
        results_comparative_modelrun.loc[ind, "Depth"] = results_df.loc[model_run, "Depth"]
        results_comparative_modelrun.loc[ind, "min_samples_leaf"] = results_df.loc[
            model_run, "min_samples_leaf"]
        results_comparative_modelrun.loc[ind, "min_samples_split"] = results_df.loc[
            model_run, "min_samples_split"]
        results_comparative_modelrun.loc[ind, "ccp_alpha"] = results_df.loc[model_run, "ccp_alpha"]
        results_comparative_modelrun.loc[ind, "Number of Estimators"] = results_df.loc[
            model_run, "Number of Estimators"]
        results_comparative_modelrun.loc[ind, "Available Features per Estimator"] = results_df.loc[
            model_run, "Percentage of Features"]
        results_comparative_modelrun.loc[ind, "Mean_Approximated_Values[%]"] = features_df[
            "NA-Values [%]"].mean()
        for number in range(0, 10):
            results_comparative_modelrun.loc[ind, f" Top {number + 1} feature"] = top_features[number]

results_comparative_modelrun.to_csv(path_results / f"Overview_Random_Forest_Parameters.csv", index=False)
